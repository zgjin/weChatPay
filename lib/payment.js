...
class Payment {
  ...
  getReadyPayParams = async (order) => {
    console.log('getReadyPayParams.params', order);
    ...
    const payParams = Object.assign({}, {
      ...
      nonce_str: util._generateNonceStr(),
      package: packageVal
    });
    payParams.paySign = util._getSign(payParams, this.config.partner_key);
  ...
  getWcPayConfirmRequest = async (req) => {
    ...
    const { error, body } = await util.validateBody(rawBody, this.config.partner_key);
    return { error, body };
  }
